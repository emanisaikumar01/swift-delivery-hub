      ...
      {isLoading ? (
        ...
        <div className="space-y-6">
          {orders?.map((order) => {
            ...
            // Assuming order.items is properly typed from schema, otherwise cast it
            // order.items is JSONB, so it might need explicit typing here if not handled by Zod in response
            // For now, assuming standard array based on schema
            const itemCount = Array.isArray(order.items) ? order.items.reduce((acc: number, item: any) => acc + item.quantity, 0) : 0;
                ...
                <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
                  <div className="space-y-1">
                    <div className="flex items-center gap-3">
                      ...
                      <div className={cn("flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-semibold uppercase tracking-wide", getStatusColor(order.status))}>
                        <StatusIcon className="h-3 w-3" />
                        {order.status}
                      </div>
                    </div>
                    <p className="text-sm text-muted-foreground">
                      {order.createdAt ? format(new Date(order.createdAt), "PPP 'at' p") : "Just now"}
                    </p>
                  </div>
